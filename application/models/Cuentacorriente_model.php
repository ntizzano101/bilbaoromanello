...
class Cuentacorriente_model extends CI_Model {
    ...
    public function __construct()
        ...
        public function nombre($p,$t=''){
				if($t==''){
						$t='C';
						$query=$this->db->get_where('clientes',array("id_cliente"=>$p));				
				$a= $query->result();
				if(count($a)>0)
					return $a[0]->nombre ." ". $a[0]->apellido ;
				return "No Existe Cliente";
					}	
				else{
						$t='P';
						$query=$this->db->get_where('proveedores',array("id_proveedor"=>$p));
				//echo $this->db->queries[0];die;
				$a= $query->result();
				if(count($a)>0)
					return $a[0]->razon_social;
        ...
        public function trae($p='',$t='')
        {
				...
				//totalv de Pedido.
		 if($t=='C'){		
			$sql="exec cc_clientes " . $p  ;	 	
			$query = $this->db->query($sql);
		}
		else{		
			$sql="exec cc_proveedores " . $p ;	 	
		 	$query = $this->db->query($sql);			
			}				
		 //  echo $this->db->queries[0];die;
		   $rta=$query->result();
